...
const itemsSample: BentoItem[] = [
  ...
  },
  {
    title: 'Queued Launch',
    meta: 'planned',
    description: 'Release-ready workflows waiting for activation',
  ...
  },
  {
    ...
    meta: 'featured',
    description: 'Highlighted intelligence module with persistent hover treatment',
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 p-4 max-w-7xl mx-auto">
      {items.map((item, index) => {
        ...
        return item.href ? (
          <Link key={`${item.title}-${index}`} to={item.href} className={cardClassName}>
            {cardContent}
          </Link>
        ) : (
